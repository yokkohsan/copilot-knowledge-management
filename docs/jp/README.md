...
## なぜ Notebook / フォルダ参照を正本管理に使わないか

Copilot Notebook やフォルダ参照は、作業中の一時的な参照・補助的な情報整理には有用です。  
ただし、本運用では、これらを前提知識や成果物の正本管理には使用しません。

実運用においては、以下の問題が起きる可能性があります。

- 内容が要約され、前提や判断の粒度が失われる
- 成果物（文章・設計・プログラムコード）が完全な形で残らない
- なぜその結論に至ったのかを、後から追えなくなる
- UI や環境によって、参照できる範囲や優先順位が変わる

本運用では、ファイルの共有 URL をプロンプトに貼り付け、Copilot が参照（添付相当）として認識できたファイルのみを対象にします。参照できない URL がある場合は、推測せず「参照不可」として扱わせます。
...
### ルール
- 参照できたファイル／参照できなかったファイルの **確認ログ** を先に出力します
...
- `[HISTORY]` は明示的に必要な場合を除き、前提復元には使いません
- 復元完了後に、通常の議論を再開します
